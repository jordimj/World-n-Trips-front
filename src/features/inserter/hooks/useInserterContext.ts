...
function useInserterContext() {
  ...
  const goNextStep = () => dispatch({ type: 'SET_ACTIVE_STEP', payload: ++state.activeStep });

  const goLastStep = () => dispatch({ type: 'SET_ACTIVE_STEP', payload: --state.activeStep });

  const setDatakind = (datakind: ImportKind) =>
    dispatch({ type: 'SET_DATA_KIND', payload: datakind });

  const setFilename = (filename: string) => dispatch({ type: 'SET_FILENAME', payload: filename });
  ...
  const setTitle = (title: string) => dispatch({ type: 'SET_JOURNAL_TITLE', payload: title });

  const setDate = (newDate: Date | null) =>
    dispatch({ type: 'SET_JOURNAL_DATE', payload: newDate });

  const setTripName = (name: string) => dispatch({ type: 'SET_TRIP_NAME', payload: name });

  const setTripSummary = (summary: string) =>
    dispatch({ type: 'SET_TRIP_SUMMARY', payload: summary });

  const setTripCover = (cover: string) => dispatch({ type: 'SET_TRIP_COVER', payload: cover });
